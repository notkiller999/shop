	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(activeCardFetch.fulfilled, (state, action) => {
				...
				cardActiveAdaptor.addOne(state, {
					id: action.payload.id,
					...action.payload.colors,
				});
			})
			.addCase(activeCardFetch.rejected, (state, action) => {
				state.cardActivity[action.payload.id] = { status: "error" };
			});
	},
});
...
export const { selectIds, selectById } = cardActiveAdaptor.getSelectors(
	(state) => state.activeCard
);

export default reducer;

export const { addData, addActiveColor, changeActiveColor, changeActiveSize } =
	actions;
